...
def solve_jacobi(A,b,e=10**(-9),aposter=True):
    R = np.triu(A,k=1)
    L = np.tril(A,k=-1)
...
def solve_seidel(A,b,e=10**(-9),aposter=True):
    R = np.triu(A,k=1)
    L = np.tril(A,k=-1)
    D_inv = np.linalg.inv(A - L - R)
    B = np.dot((-D_inv),(L+R))
...
#     A = np.random.sample((n,n))
    ...
    print()
    print(solve_jacobi(A,b,aposter=False))
    print()
    print('solve_seidel с апостериорной оценкой:')
    print()
    print(solve_seidel(A,b))
    print()
    print('solve_seidel с априорной оценкой:')
    print()
    print(solve_seidel(A,b,aposter=False))
    print()
    
    
    print('Для положительно определённой матрицы без диагонального преобладания:')
    print()
